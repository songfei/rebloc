...
class IntAction extends ReduxAction {}

class ResetAction extends ReduxAction {}

class IntBloc extends SimpleBloc<int> {
  @override
  String get moduleName => 'IntBloc';

  @override
  get initialState => 0;

  @override
  int reducer(state, action) {
...
//  @override
//  FutureOr<ReduxAction> middleware(dispatcher, state, action) {
...
class LoggerBloc extends SimpleBloc<int> {
  ReduxState lastState;

  @override
  String get moduleName => 'LoggerBloc';

  @override
  get initialState => null;

  @override
  Future<ReduxAction> middleware(dispatcher, state, action) async {
    print('${action.runtimeType} dispatched.');

    print('int state ${state.stateMap['IntBloc']}');
...
//    dispatcher(IntAction());
  ...
  @override
  FutureOr<ReduxAction> afterware(DispatchFunction dispatcher, ReduxState state, ReduxAction action) {
    if (state != lastState) {
...
/// This [Bloc] must appear before the others in the list given to the [ReduxStore].
class LimitBloc extends SimpleBloc {
...
//  static const maxLength = 10;
  ...
  FutureOr<ReduxAction> middleware(DispatchFunction dispatcher, ReduxState state, ReduxAction action) {
    if ((action is IntAction && state.stateMap['IntBloc'] == maxInt)) {
      // Cancel (or "swallow") the action if a limit would be exceeded.
      return ReduxAction.cancelled();
    }
  ...
  @override
  String get moduleName => 'LimitBloc';
}

class IntDisplayWidget extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return ViewModelSubscriber<int>(
      converter: (state) => state.stateMap['IntBloc'] as int,
...
//
//  DescriptionViewModel(SimpleAppState state) : description = state.toString() + "!";
//
//  @override
